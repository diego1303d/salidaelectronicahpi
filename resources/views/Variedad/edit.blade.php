<x-layouts.app>
    ...
    <div class="mx-auto w-full max-w-2xl space-y-6"
         x-data="{
            nombre: {{ Js::from(old('nombre', $variedad->nombre)) }},
            categoria: {{ Js::from(old('categoria', $variedad->categoria)) }},
            peso: {{ Js::from((float) old('peso_bulto_kg', $variedad->peso_bulto_kg)) }},
            activo: {{ old('activo', $variedad->activo) ? 'true' : 'false' }}
         }">
        ...
        <div>
            ...
            <h1 class="mt-1 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                Editar: {{ $variedad->nombre }}
            </h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Los cambios en el peso por bulto solo afectan capturas nuevas; los documentos ya registrados no se recalculan.
            </p>
        </div>
        ...
        {{-- ══════════════ Formulario ══════════════ --}}
        <form method="POST" action="{{ route('Variedad.update', $variedad) }}"
                ...
                <div class="flex items-center gap-3 rounded-lg border border-dashed border-gray-300 bg-gray-50 px-4 py-3
                    ...
                    <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold"
                        ...
                        <span class="inline-flex h-2 w-2 rounded-full"
                              :class="activo ? 'bg-emerald-500' : 'bg-gray-400'"></span>
                        <span x-text="activo ? 'Activa' : 'Inactiva'"></span>
                    </span>
                </div>
            </div>
