<x-layouts.app>
    ...
    <!-- Page Title -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Perfil') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('Actualiza tu nombre y correo electrónico') }}</p>
    </div>
                ...
                <div
                    ...
                    <div class="p-6">
                        ...
                        <form class="max-w-md mb-10" action="{{ route('settings.profile.update') }}" method="POST">
                            ...
                            @method('PUT')
                            <div class="mb-4">
                                <x-forms.input label="Nombre" name="name" type="text"
                                    value="{{ old('name', $user->name) }}" />
                            </div>

                            <div class="mb-6">
                                <x-forms.input label="Correo" name="email" type="email"
                                    value="{{ old('email', $user->email) }}" />
                            </div>

                            <div>
                                <x-button type="primary">{{ __('Guardar') }}</x-button>
                            </div>
                        </form>
                        ...
                        <div class="border-t border-gray-200 dark:border-gray-700 pt-6 mt-6">
                            ...
                            <p class="text-gray-600 dark:text-gray-400 mb-4">
                                {{ __('Elimina tu cuenta y todos sus recursos') }}
                            </p>
                            <form action="{{ route('settings.profile.destroy') }}" method="POST"
                                ...
                                @csrf
                                @method('DELETE')
                                <x-button type="danger">{{ __('Eliminar ') }}</x-button>
                            </form>
                        </div>
                    </div>
                </div>
